...
export const downloadBillPDF = async (billId) => {
    try {
        const response = await axiosInstance.get(`/bills/${billId}/download`, {
...
export const getBillingReport = async (type, year, month) => {
    try {
        ...
        const response = await axiosInstance.get('/bills/report', { params });
        return response.data.data;
    } catch (error) {
...
export const downloadBillingReport = async (type, year, month, format = 'pdf') => {
    try {
        ...
        const response = await axiosInstance.get('/bills/report/download', {
            params,
            responseType: 'blob',
        });
