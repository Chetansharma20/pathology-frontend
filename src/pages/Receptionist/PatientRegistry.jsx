...
import { getPatients, createPatient, updatePatient, deletePatient, getPatientById, getDailyPatients } from '../../api/receptionist/patient.api';
...
const PatientRegistry = () => {
    ...
    const { refreshTodayPatients, metrics } = useData();
    ...
    // Daily analytics state (COMMENTED OUT)
    ...
    // Fetch daily patient data (COMMENTED OUT)
    // const fetchDailyPatients = async () => {
    //     try {
    //         setDailyLoading(true);
    //         const response = await getDailyPatients(selectedYear, selectedMonth);
    //         if (response.data) {
    //             setDailyData(response.data);
    //         }
    //     } catch (error) {
    //         showToast('Failed to fetch daily patient data', 'error');
    //         console.error(error);
    //     } finally {
    ...
    // Fetch daily data when month/year changes (COMMENTED OUT)
    // useEffect(() => {
    //     if (showDailyAnalytics) {
    //         fetchDailyPatients();
    //     }
    // }, [selectedMonth, selectedYear, showDailyAnalytics]);
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex gap-2">
                    {/* Daily Analytics Button (COMMENTED OUT)
                    <button
                        onClick={() => setShowDailyAnalytics(!showDailyAnalytics)}
                        className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-sm transition-colors"
                        style={{
                            backgroundColor: showDailyAnalytics ? 'var(--accent-purple)' : 'var(--button-secondary)',
                            color: showDailyAnalytics ? 'var(--text-inverse)' : 'var(--text-primary)',
                            border: showDailyAnalytics ? 'none' : '1px solid var(--border-primary)'
                        }}
                    >
                        <Database size={18} />
                        {showDailyAnalytics ? 'Hide Analytics' : 'Daily Analytics'}
                    </button>
                    */}
                    <button
                        onClick={() => {
            ...
            {/* Daily Analytics Section - COMMENTED OUT - See git history to restore */}
